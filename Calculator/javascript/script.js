$(document).ready(function () {
    ...
    $('#theme-purple').click(function () {
        $('#main').css({"background": "linear-gradient(to bottom right, #91929e, #444991"});
        $('.op').css({"color": "lightslategray"});
        $('.num-button').css({"color": "purple"});
    });
    // red
    $('#theme-red').click(function () {
        $('#main').css({"background": "linear-gradient(to bottom right, #ead6d3 , #af0e29"});
        $('.op').css({"color": "orange"});
        $('.num-button').css({"color": "red"});
    });
    // blue
    $('#theme-blue').click(function () {
        $('#main').css({"background": "linear-gradient(to bottom right, white , #0e238e"});
        $('.op').css({"color": "dodgerblue"});
        $('.num-button').css({"color": "blue"});
    });
    // blue green
    $('#theme-bluegreen').click(function () {
        $('#main').css({"background": "linear-gradient(to bottom,  #a5ffaa , #0e23af)"});
        $('.op').css({"color": "lightseagreen"});
        $('.num-button').css({"color": "bluegreen"});
    });
    // green
    $('#theme-green').click(function () {
        $('#main').css({"background": "linear-gradient(to bottom right, #f7db0c , #09c125"});
        $('.op').css({"color": "limegreen"});
        $('.num-button').css({"color": "green"});
    });
    // black
    $('#theme-black').click(function () {
        $('#main').css({"background": "linear-gradient(to bottom right, #dde2d5, #939989"});
        $('.op').css({"color": "#434343"});
